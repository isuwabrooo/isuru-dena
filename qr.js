...
async function isuru () {
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('isuru')}${chalk.blue.bold('Whatsapp-Bot')}
...
${chalk.blue.italic('📶 Connecting to Whatsapp Web... Please Wait ❗')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('isuru Bailyes-Api: '), Session.createStringSession(conn.base64EncodedAuthInfo())
        );
        
        if (!fs.existsSync('config.env')) {
            fs.writeFileSync('config.env', `ISURU_SESSION="${st}"`);
        }
